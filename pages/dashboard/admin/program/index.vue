          ...
          <template v-slot:header>
            <div class="d-flex flex-row mb-5">
              <v-btn
                ...
                class="mt-3 mr-4"
                color="primary"
                @click="$router.push('/dashboard/admin/program/add')"
              >
                <v-icon>add</v-icon>
                Buat
              </v-btn>
                ...
                <v-card class="fill-height" elevation="5">
                  ...
                  <v-card-actions>
                    <v-btn
                      ...
                      :to="`/dashboard/admin/program/${item.id}`"
                      text
                    >
                      <v-icon>edit</v-icon>
                    </v-btn>
...
export default {
  ...
  computed: {
    page: {
      get() {
        return this.$store.state.admin.program.pagination.page
      },
      set(newValue) {
        this.$store.commit('admin/program/exportPaginationPage', newValue)
      },
    },
    limit: {
      get() {
        return this.$store.state.admin.program.pagination.limit
      },
      set(newValue) {
        this.$store.commit('admin/program/exportPaginationLimit', newValue)
      },
    },
    order: {
      get() {
        return this.$store.state.admin.program.pagination.order
      },
      set(newValue) {
        this.$store.commit('admin/program/exportPaginationOrder', newValue)
      },
    },
    programData() {
      return this.$store.state.admin.program.pagination.data
    },
  },
  methods: {
    serverBaseUrl() {
    ...
    async destroy(id) {
      ...
      if (notif.isConfirmed) {
        const response = await this.$store.dispatch('admin/program/destroy', id)
  ...
  async fetch() {
    await this.$store.dispatch('admin/program/pagination', {
      page: this.page,
      limit: this.limit,
      order: this.order,
    })
