    ...
    <v-card elevation="3" class="mb-4">
      ...
      <v-container>
        ...
        <v-btn
          type="button"
          color="primary"
          to="/dashboard/superadmin/user/rule/add"
        >
          <v-icon>add</v-icon>
          Tambah
        </v-btn>
    ...
    <v-card elevation="3" class="mt-3">
      ...
      >
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-icon
            class="mr-2"
            @click="$router.push(`/dashboard/superadmin/user/rule/${item.id}`)"
          >
            edit
          </v-icon>
...
export default {
  ...
  computed: {
    payload() {
      return this.$store.state.superadmin.rule.pagination
    },
    ruleData() {
      return this.$store.state.superadmin.rule.pagination.data
    },
  },
  methods: {
    updatePage(event) {
