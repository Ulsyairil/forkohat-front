...
export default {
  ...
  async fetch() {
    try {
      let url = `${this.baseurl.dev}/superadmin/event?event_id=${this.$route.params.id}`
      const config = {
        headers: {
  ...
  methods: {
    async deleteEvent(id) {
      try {
        ...
        let url = `${this.baseurl.dev}/superadmin/event`
        let payload = {
          event_id: id,
        }
        const config = {
    ...
    async restoreEvent(id) {
      try {
        ...
        let url = `${this.baseurl.dev}/superadmin/event/restore`
        const config = {
          headers: {
            Authorization: `${this.auth.token.type} ${this.auth.token.token}`,
          },
    ...
    async dumpEvent(id) {
      try {
        ...
        let url = `${this.baseurl.dev}/superadmin/event/dump`
        const config = {
          headers: {
            Authorization: `${this.auth.token.type} ${this.auth.token.token}`,
          },
    ...
    async destroyFile(id) {
      try {
        $.LoadingOverlay('show')
        let url = `${this.baseurl.dev}/superadmin/event/file`
        let payload = {
          file_id: id,
        }
        const config = {
    ...
    async formSubmit() {
      try {
        ...
        const url = `${this.baseurl.dev}/superadmin/event`
        const config = {
          headers: {
            Authorization: `${this.auth.token.type} ${this.auth.token.token}`,
          },
